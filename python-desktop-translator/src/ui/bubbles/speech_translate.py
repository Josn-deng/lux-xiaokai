...
class SpeechTranslateBubble(QtWidgets.QWidget):
    def __init__(self, config, client):
        ...
        try:
            # 使用正确的相对路径
            icon_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "icons", "speech_translate.png")
            if os.path.exists(icon_path):
                self.setWindowIcon(QtGui.QIcon(icon_path))
            else:
                # 调试信息
                print(f"语音翻译图标未找到: {icon_path}")
        except Exception as e:
            print(f"设置语音翻译图标时出错: {e}")
    ...
    def setup_chat_ui(self):
        ...
        # 标题
        title_label = QtWidgets.QLabel("Speech Translation")
        ...
        self.input_text.setPlaceholderText("Paste speech recognition result here...")
        self.input_text.setMaximumHeight(100)
        self.input_text.setFixedHeight(80)
        
        button_layout = QtWidgets.QHBoxLayout()
        button_layout.setContentsMargins(0, 0, 0, 0)
        self.translate_button = QtWidgets.QPushButton("Translate Speech")
        self.translate_button.clicked.connect(self._translate_current)
    ...
    def _translate_current(self):
        recognized = self.input_text.toPlainText().strip()
        if not recognized:
            QtWidgets.QMessageBox.warning(self, "Empty Input", "Please paste speech recognition result.")
            return
            
        # 添加用户消息到对话历史
        self.add_message("user", recognized)
        ...
        try:
            ...
            # 构造请求参数
            payload = self.config.build_speech_translation_prompt(recognized)
            ...
            self.translate_button.setText("Translate Speech")
